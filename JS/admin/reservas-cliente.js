document.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    misReservas = reservasGuardadas ? JSON.parse(reservasGuardadas) : [];
  } catch (error) {
    localStorage.removeItem("mis_reservas");
  }

  if (!Array.isArray(misReservas)) {
    misReservas = [];
  }

  misReservas = misReservas.map((reserva, index) => ({
    ...reserva,
  ...
  if (contenedorProximas) {
    ...
    misReservas.forEach((reserva, index) => {
      const tarjetaHTML = `
        <div class="tarjeta-reserva" data-estado="${escaparHtml(reserva.estado || "Confirmada")}" style="background: #181c24; border: 1px solid #2a2f3a; border-radius: 8px; padding: 15px; margin-bottom: 15px; display: flex; align-items: center; justify-content: space-between; gap: 15px;">
          ...
          <div style="flex: 1;">
            <h3 style="margin: 0 0 5px 0; color: #fff; font-size: 1.1rem;">${escaparHtml(reserva.nombre || 'Cancha Deportiva')}</h3>
            <p style="margin: 0; color: #aaa; font-size: 0.85rem;">Ubicación: ${escaparHtml(reserva.ubicacion || 'Ubicación no especificada')}</p>
            <small style="color: #107c41; font-weight: bold;">Fecha: ${escaparHtml(reserva.fecha || '')} - Hora: ${escaparHtml(reserva.hora || reserva.horario || '')} (${escaparHtml(reserva.duracion || '')})</small>
          </div>
  ...
  if (btnCancelar) {
    btnCancelar.addEventListener("click", () => {
      ...
      const reservasActualizadas = misReservas.map((reserva) => {
